...
class sql():
    ...
    def ReadAccounts(self):
        ...
        for row in self.cursor:
            ...
            'recover': "0",
            'characters': []}
        LOG.info(Console.Load.value.format(number=len(accounts), table="users", time=round(time.perf_counter()-start, 2)))
        return accounts
    ...
    def StoreItems(self):
        ...
        for row in self.cursor:
            ...
            'service': row[9],
            'maxskill': row[10]})
        LOG.info(Console.Load.value.format(number=len(store), table="store", time=round(time.perf_counter()-start, 2)))
        return store
    ...
    def ReadRedeemCode(self):
        ...
        for row in self.cursor:
            ...
            'usedto': row[1]}
        LOG.info(Console.Load.value.format(number=len(redeemcodes), table="redeemcodes", time=round(time.perf_counter()-start, 2)))
        return redeemcodes
    ...
    def ReadRealms(self):
        ...
        LOG.info(Console.Load.value.format(number=len(realmlists), table="realmlist", time=round(time.perf_counter()-start, 2)))
        return realmlists
    ...
    def ReadLinks(self):
        ...
        LOG.info(Console.Load.value.format(number=len(navlinks), table="navlinks", time=round(time.perf_counter()-start, 2)))
        return navlinks
    ...
    def GetBuyHistory(self, email):
        ...
        self.webcore.disconnect()
        LOG.info(Console.Load.value.format(number=len(history), table="history", time=round(time.perf_counter()-start, 2)))
        return history

    def Register(self, firstname, lastname, email, password, regdate, phonenumber, username):
        self.webcore.reconnect()
        self.cursor.execute("INSERT INTO users (firstname, lastname, email, password, token, gold, regdate, status, phonenumber, username, code, count) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)", (firstname, lastname, email, password, 0, 0, regdate, "فعال", phonenumber, username, 0, 0))
        self.webcore.commit()
        accounts[email] = {'firstname': firstname, 'lastname': lastname, 'email': email, 'password': password, 'token': 0, 'gold': 0, 'regdate': regdate, 'status': "فعال", 'phonenumber': phonenumber, 'username': username, 'code': 0, 'count': 0}
